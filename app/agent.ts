...
): Promise<{ message: StoredAssistantMessage; streamed: boolean }> {
  const requestMessages = trimMessages(
    ctx.messages,
    ctx.options.maxHistoryChars ?? DEFAULT_MAX_HISTORY_CHARS,
  );
  const useStreaming = Boolean(ctx.options.onAssistantTextDelta);
      ...
      if (useStreaming) {
        const message = await streamChatCompletion(
          ...
          async (delta) => {
            ...
            streamed = true;
            await ctx.options.onAssistantTextDelta!(delta);
          },
          ctx.options.signal,
        );
...
export async function runAgent(
  ...
  const ctx: AgentRequestContext = {
    ...
    model,
    messages,
    activeTools: options.tools ?? coreTools,
    options,
  };
  let temperature = 0.2;
  ...
  while (true) {
    ...
    const { message, streamed } = await requestAssistantMessage(
      ctx,
      temperature,
    );
    ...
    await emitAssistantText(message, useStreaming, streamed, options);

    if (!message.tool_calls?.length) {
      return typeof message.content === "string" ? message.content : "";
    }

    await runToolCalls(message, messages, options);
  }
}
